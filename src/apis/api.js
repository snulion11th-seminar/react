...
export const myPage = async () => {
  const response = await instance.get("/account/myinfo/");
  if (response.status === 200) {
...
export const getMyInfo = async () => {
  const response = await instanceWithToken.get("/account/myinfo/");
  if (response.status === 200) {
    console.log(response.data);
  } else {
    console.log("[ERROR] error with user info");
  }
  return response.data;
};
...
export const updateUserInfo = async (data) => {
  // console.log(data)
  const response = await instanceWithToken.put("/account/myinfo/", data);
